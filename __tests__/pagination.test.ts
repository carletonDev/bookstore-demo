...
describe("encodeCursor", () => {
  it("returns a base64url string", () => {
    const cursor = encodeCursor(
      ...
      "5dc6c71c-cf29-43fe-9581-12f390ff26b0",
    );
...
describe("decodeCursor", () => {
  ...
  it("throws on valid base64 with missing id field", () => {
    const badCursor = Buffer.from(JSON.stringify({ title: "ok" })).toString(
      "base64url",
    );
    expect(() => decodeCursor(badCursor)).toThrow("Invalid pagination cursor");
  });
  ...
  it("throws on valid base64 with non-string fields", () => {
    const badCursor = Buffer.from(
      JSON.stringify({ title: 123, id: "ok" }),
    ).toString("base64url");
    expect(() => decodeCursor(badCursor)).toThrow("Invalid pagination cursor");
  });
...
describe("buildCursorFilter", () => {
  ...
  it("escapes double quotes in title", () => {
    ...
    expect(filter).toBe(
      'title.gt."Book \\"Quoted\\"",and(title.eq."Book \\"Quoted\\"",id.gt.x)',
    );
  });
  ...
  it("escapes backslashes in title", () => {
    ...
    expect(filter).toBe(
      'title.gt."Back\\\\slash",and(title.eq."Back\\\\slash",id.gt.y)',
    );
  });
});
