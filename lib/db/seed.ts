...
async function seedAuthors(): Promise<string[]> {
  const rows = Array.from({ length: 50 }, () => {
    ...
    return {
      ...
      last_name: lastName,
      bio:
        `${firstName} ${lastName} is a software engineer specializing in ${spec} ` +
...
async function seedBooks(publisherIds: string[]): Promise<string[]> {
  ...
  const rows = titles.map((title) => ({
    ...
    description:
      `A comprehensive guide to ${title.replace(/:.+/, "").toLowerCase()}. ` +
...
async function seedBookAuthors(
  ...
  for (const bookId of bookIds) {
    ...
    for (const authorId of selected)
      rows.push({ book_id: bookId, author_id: authorId });
  }
...
async function seedBookGenres(
  ...
  for (const bookId of bookIds) {
    ...
    for (const genreId of selected)
      rows.push({ book_id: bookId, genre_id: genreId });
  }
